...
public class SampleUserColumnFamily extends BaseTable
{
		...
		private static byte[] accountFamilyForeignNameBytes;
		private static byte[] accountsUsersForeignNameBytes;
		
		//Each fields name in bytes
		...
		private static byte[] infoFirstNameBytes;
		...
		//Many 2 Many Columns
		protected Many2Many<DummyAccounts> accounts;

		//Field Changed Flags
		protected boolean password_Changed = false;
		protected boolean salt_Changed = false;
		protected boolean enabled_Changed = false;

		protected boolean firstName_Changed = false;
		protected boolean lastName_Changed = false;
		protected boolean email_Changed = false;
		...
		public SampleUserColumnFamily(OCMConnection connection, String userID)
		{
			...
			accounts = new Many2Many<DummyAccounts>();
			...
			if(familyNameBytes == null)
			{
				...
				//Convert each supper columns name
				...
				infoFirstNameBytes = fromString("firstName");
			}
		...
		public void delete() throws IllegalArgumentException, NotFoundException, TException, Exception
		{
			...
			try
			{
				...
				//Delete the other side of Many 2 Many relationships
				Enumeration<String> enumeration = accounts.getColumns().keys();
		...
		public void loadAll(List<SuperColumn> data)
		{
			...
			while(iterator.hasNext())
			{
				...
				if(Arrays.equals(superCol.getName(), authSuperNameBytes))
		...
		private void loadAuth(List<Column> cols)
		{
			...
			while(iterator.hasNext())
			{
				...
				if(Arrays.equals(col.getName(), authPasswordNameBytes))
				{
		...
		private void loadInfo(List<Column> cols)
		{
			...
			while(iterator.hasNext())
			{
				...
				if(Arrays.equals(col.getName(), infoFirstNameBytes))
				{
		...
		{			
			Dictionary<String, DummyAccounts> data = accounts.getColumns();
			...
			while(iterator.hasNext())
			{
				...
				//Get each entry (Don't load it yet)
				DummyAccounts entry = new DummyAccounts(connection, key);
		...
		public void save() throws Exception
		{
			...
			try
			{
				...
				{
					//Add the changed column
					cols.add(new Column(infoFirstNameBytes, fromString(firstName), System.currentTimeMillis()));
		...
		public Many2Many<DummyAccounts> getAccounts()
		{
			return accounts;
		}

		public static SampleUserColumnFamilyScanner getScanner(OCMConnection connection)
		{
			return new SampleUserColumnFamilyScanner(connection);
		}
		

		public static class SampleUserColumnFamilyScanner extends BaseTableScanner
		{			
			public SampleUserColumnFamilyScanner (OCMConnection connection)
			{
